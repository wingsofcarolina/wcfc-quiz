...
public class Populate {
  ...
  private void multipleChoice(Category category, List<String> attributes) {
    ...
    attributeList =
      ListUtils.union(
        attributeList,
        Arrays.asList(Attribute.difficulty_attributes.get(pick(3)))
      );
  ...
  private void sopQuestions() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      attributes.add(
        Attribute.level_attributes.get(pick(Attribute.level_attributes.size()))
      );
      ...
      question =
        new Question(
          ...
          Category.SOP,
          attributes,
          getWordsWithBlanks(count, 10, 20),
          lorem.getWords(1, 3),
          answers,
          lorem.getParagraphs(1, 2)
        );
