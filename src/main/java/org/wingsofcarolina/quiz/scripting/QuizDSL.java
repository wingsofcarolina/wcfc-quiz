...
public abstract class QuizDSL extends Script {
  ...
  public void end() {
    ...
    if (section != null) {
      ...
      for (Question question : questions) {
        ...
        if (
          quiz.getMaxCount() != null && quiz.getQuestions().size() >= quiz.getMaxCount()
        ) {
          break;
        } else {
      ...
      if (context.getTestRun()) {
        System.out.println(
          "ERROR : Attempted to add an empty/null section to the quiz. Why?<br>"
        );
      } else {
  ...
  public void select(Integer count, Pool pool) {
    ...
    if (section != null && pool != null) {
      if (count <= pool.size()) {
          ...
          if (!excluded && !alreadySelected && !deleted && !quarintined && !empty) {
            ...
            if (context.getTestRun()) {
              System.out.println(
                ...
                " : " +
                printList(entity.getAttributes()) +
                "<br>"
              );
            }
          } else {
            if (context.getTestRun()) {
              ...
              if (quarintined) previous =
                failOut(previous, "<strong>quarintined</strong> ");
              if (alreadySelected) previous =
                failOut(previous, "<strong>already selected</strong> ");
        ...
        if (context.getTestRun()) {
          System.out.println(
            "ERROR : Selection attempted with an undersize pool, wanted " +
            count +
            " and only had " +
            pool.size() +
            " in section " +
          ...
          LOG.info(
            "Selection attempted with an undersize pool, wanted {} and only had {} in section {}",
            count,
            pool.size(),
            section.getName()
          );
      ...
      if (context.getTestRun()) {
        System.out.println(
          "ERROR : Attempted to add questions outside of a section.<br>"
        );
      } else {
      ...
      if (question != null) {
        ...
        if (context.getTestRun()) {
          System.out.println(
            ...
            " : " +
            printList(question.getAttributes()) +
            "<br>"
          );
        }
      } else {
  ...
  public void require(Pool pool) {
    for (Question question : pool.getQuestionList()) {
      ...
      if (question != null) {
        ...
        if (context.getTestRun()) {
          System.out.println(
            "ERROR : Impossible error, got a null question from a pool<br>"
          );
        } else {
  ...
  public Pool getRequiredWith(List<String> attributes) {
    ...
    if (context.getTestRun()) {
      System.out.println(
        "Required question count for '" + attributes + "' is " + pool.size() + "<br>"
      );
    }
    return pool;
  }
  ...
  public Pool filterIn(Pool pool, List<String> attributes) {
    ...
    for (
      ListIterator<Question> iter = pool.getQuestionList().listIterator();
      iter.hasNext();
    ) {
  ...
  private Question resolve(Question question) {
    while (question != null && question.isSuperseded()) {
      ...
      if (tmp == null) {
        System.out.println(
          "<br>Found a null entity from resolve(" +
          question.getQuestionId() +
