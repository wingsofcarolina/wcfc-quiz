...
public class QuizAPI {
  ...
  public Response explore(
    @CookieParam("quiz.token") Cookie cookie,
  ...
  public Response resetPassword(
    ...
    Flash.add(
      Flash.Code.SUCCESS,
      "Password for " + user.getName() + " reset to the default 'REDACTED'."
    );
    return new RedirectResponse(Pages.HOME_PAGE)
      .cookie(authUtils.generateCookie(requester))
      .build();
  }
  ...
  public Response login(
    ...
    try {
      if (
        storedPassword == null || !HashUtils.validatePassword(password, storedPassword)
      ) {
    ...
    Slack
      .instance()
      .sendMessage("Logged in  : " + user.getName() + " (" + user.getEmail() + ")");
  ...
  public Response changePassword(
    ...
    if (password == null || passwordVerify == null) {
      Flash.add(
        Flash.Code.ERROR,
        "No password or verification password provided, try again."
      );
    } else if (!password.equals(passwordVerify)) {
      Flash.add(Flash.Code.ERROR, "Passwords do not match.");
    }
    ...
    return new RedirectResponse(Pages.HOME_PAGE)
      .cookie(authUtils.generateCookie(user))
      .build();
  }
  ...
  public Response retrieve(
    ...
    @FormParam("type") String type
  ) throws AuthenticationException, IOException, QuizBuildException, URISyntaxException {
    String output = "";
    Jws<Claims> claims = authUtils.validateUser(cookie.getValue());
    User user = User.getWithClaims(claims);
  ...
  @Path("globalKey")
  @Produces("text/html")
  public Response globalKey(@CookieParam("quiz.token") Cookie cookie)
    ...
    Slack
      .instance()
      .sendMessage(
        "Global Database Quiz Key  retrieved by " +
        user.getName() +
        " at " +
        dateFormatGmt.format(new Date())
      );
  ...
  @GET
  @Path("backup")
  public Response backupDatabase(@CookieParam("quiz.token") Cookie cookie)
    ...
    Slack
      .instance()
      .sendMessage("Backup database requested at " + dateFormatGmt.format(new Date()));
    ...
    Flash.add(
      Flash.Code.SUCCESS,
      "Saved " + r_count + " recipes and " + q_count + " questions from the database."
    );
    return new RedirectResponse(Pages.HOME_PAGE)
      .cookie(authUtils.generateCookie(user))
      .build();
  ...
  @GET
  @Path("restore")
  public Response restoreDatabase(@CookieParam("quiz.token") Cookie cookie)
    throws AuthenticationException {
    int q_count = 0;
    int r_count = 0;
    ...
    Slack
      .instance()
      .sendMessage(
        "Restore of all questions requested at " + dateFormatGmt.format(new Date())
      );
    LOG.info(
      "Restore of all questions requested at {}",
      dateFormatGmt.format(new Date())
    );
  ...
  public Response uploadFile(
    ...
    while ((entry = stream.getNextEntry()) != null) {
      ...
      try {
        if (entry.getName().startsWith("quizdata/r-")) {
          ...
          Question question = objectMapper.readValue(
          ...
          LOG.error(
            "Found an entry in the upload file that can't be deserialized : {}",
            entry.getName()
          );
        }
    ...
    return Response
      .status(200)
      .entity("Zip file uploaded by " + user.getName() + "\n")
      .build();
  }
  ...
  public Response uploadImage(
    ...
    if (!file.exists()) {
      try {
        java.nio.file.Path outputPath = FileSystems
          .getDefault()
  ...
  public Response addQuestion(
    ...
    if (typeName.contentEquals("fib")) {
      ...
      if (correct == null || correct.size() == 0) {
        Flash.add(
          Flash.Code.ERROR,
          "A question may not be saved with no correct answer set."
        );
        return new RedirectResponse(Pages.HOME_PAGE)
          .cookie(authUtils.generateCookie(user))
          .build();
    ...
    return new ViewQuestionResponse(newQuestion.getQuestionId())
      .cookie(authUtils.generateCookie(user))
      .build();
  }
  ...
  public Response updateQuestion(
    ...
    @FormParam("quarantined") Boolean quarantined,
    @FormParam("required") Boolean required,
    @FormParam("type") String typeName,
    @FormParam("category") String categoryName,
    @FormParam("question") String question,
    ...
    if (!typeName.contentEquals("BLANK")) {
      if (correct.size() > 0) {
        ...
        Flash.add(
          Flash.Code.ERROR,
          "Question with ID " + questionId + " had no answers provided. Not updated."
        );
        return new ViewQuestionResponse(original.getQuestionId())
          .cookie(authUtils.generateCookie(user))
          .build();
      }
    }
      ...
      if ((!original.isDeployed()) || overwrite == true) {
        ...
        if (original.getExclusionId() != exclusionId) {
          ...
          if (original.getExclusionId() != 0 && original.getExclusionId() != null) {
            ExclusionGroup oldGroup = ExclusionGroup.getByGroupId(
              original.getExclusionId()
            );
        ...
        if (changed) {
          Flash.add(
            Flash.Code.SUCCESS,
            "Updated existing question with ID : " + original.getQuestionId()
          );
        } else {
          Flash.add(
            Flash.Code.SUCCESS,
            "No changes made to question with ID : " + original.getQuestionId()
          );
        }
      } else {
        ...
        return new ViewQuestionResponse(q.getQuestionId())
          .cookie(authUtils.generateCookie(user))
          .build();
      }
    } else {
  ...
  private Question createQuestion(
    ...
    Flash.add(
      Flash.Code.SUCCESS,
      "Created new question with ID : " + newQuestion.getQuestionId()
    );
    return newQuestion;
  }
  ...
  @Path("exclusion")
  @Produces("text/html")
  public Response createExclusion(
    @CookieParam("quiz.token") Cookie cookie,
    @FormParam("name") String name,
  ...
  public Response deleteQuestion(
    ...
    if (question.isSuperseded() || question.isDeployed() == true) {
      Flash.add(
        ...
        " is either superseded or deployed so is marked for future deletion."
      );
      ...
      if (question.getExclusionId() != null && question.getExclusionId() != 0) {
        ExclusionGroup exclusionGroup = ExclusionGroup.getByGroupId(
          question.getExclusionId()
        );
  ...
  public Response updateRecipe(
    ...
    if (cookie != null) {
      ...
      return new RedirectResponse(Pages.RECIPE_PAGE, recipeId)
        .cookie(authUtils.generateCookie(user))
        .build();
    } else {
  ...
  public Response downloadBackup(@CookieParam("quiz.token") Cookie cookie)
    throws AuthenticationException {
    if (cookie != null) {
      Jws<Claims> claims = authUtils.validateUser(cookie.getValue(), Privilege.ADMIN);
      User.getWithClaims(claims);
      ...
      Slack
        .instance()
        .sendMessage(
          "Download of all questions requested at " + dateFormatGmt.format(new Date())
        );
      LOG.info(
        "Download of all questions requested at {}",
        dateFormatGmt.format(new Date())
      );
